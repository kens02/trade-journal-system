...
- DB初期化時の `navigator.storage.persist()` 呼び出し(結果 granted/denied を appMeta に記録。最終バックアップ日時の表示・7日超過警告はP4)
...
- **廃止/再有効化**: 一覧の各ルールに status 切替操作を設ける。`retired` のルールは取引入力フォームの選択肢に表示しない(既存の紐付け・履歴・集計は保持)
- **物理削除**: 一覧に削除ボタンを設ける。そのルールの全RuleVersionにTradeRuleLinkが1件も存在しない場合のみ、確認ダイアログを経て Rule+全RuleVersion を同一トランザクションで削除する。紐付けが1件でも存在する場合はエラーを表示し、「廃止」の利用を案内する
- 取引からの参照有無に関わらず、versionは編集できないことをUI上も徹底する
...
- [ ] 取引が紐付いたルールを削除しようとするとエラーになり、「廃止」が案内される
- [ ] 紐付けゼロのルールは確認ダイアログを経て物理削除でき、全RuleVersionも同時に消える
- [ ] 廃止(retired)にしたルールが取引入力フォームの選択肢から消え、既存の集計には残る
...
- [ ] 初回起動時に `navigator.storage.persist()` が呼ばれ、結果が appMeta に記録される
